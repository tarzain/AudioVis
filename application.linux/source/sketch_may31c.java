...
public class sketch_may31c extends PApplet {
...
//  max=0;
  ...
  if(showStars){
    for(int i=fft.specSize(); i>0; i-=10){
        rect(i+1,(height-fft.getBand(i)*4)-height/2,1.5f,1.5f);
        fill(255);
    }  
    for(int i=0; i<fft.specSize(); i+=10){
        rect((width-i),(height-fft.getBand(i)*4)-height/2,1.5f,1.5f);
        fill(255);
    }
  }  
...
//  text("The window being used is: " + windowName, 5, 20);
  ...
  text("The current time in the song is: "+floor((timerCtr*avgFR*2.13f/1000)/60)+":"+(timerCtr*avgFR*2.28f/1000)%60, 5,80);
  }  
  z-=0.5f;
...
public void changeGlobeColor(float col){
	col=abs(col);
        colorMode(HSB);
	int c=color(col,col,col);
...
//   System.out.println(x+" "+y+" "+z); 
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#F0F0F0", "sketch_may31c" });
  }
}
